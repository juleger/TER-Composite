...
void Config::loadFromFile(const string& filename) {
    ...
    std::string et = getString("element_type", "P1");
    ...
    hasPores = (params.find("Young_modulus_pore") != params.end());
    E_pore = getDouble("Young_modulus_pore", E);
    nu_pore = getDouble("Poisson_ratio_pore", nu);
    rho_pore = getDouble("density_pore", rho);
...
void Config::print() const {
    ...
    cout << "Matrice (1) - E: " << E << " Pa, nu: " << nu << ", rho: " << rho << " kg/m3" << endl;
    if (hasFiber) {
        cout << "Fibre (2) - E: " << E_fiber << " Pa, nu: " << nu_fiber << ", rho: " << rho_fiber << " kg/m3" << endl;
    }
    if (hasPores) {
        cout << "Porosités (3) - E: " << E_pore << " Pa, nu: " << nu_pore << ", rho: " << rho_pore << " kg/m3" << endl;
    }
